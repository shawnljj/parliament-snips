...
`ingest/debates.py` turns the archive into the normalized retrieval corpus the RAG layer
reads. It is a **mapper, not a second scraper**: it reuses `scraper/`'s cache and paths and
writes one JSON object per line conforming to `docs/schema.json`. Field names, enums and
required sets come from that schema — do not invent fields here.

```bash
# Smoke run: live fetch, first 500 documents. This is the acceptance command.
python3 ingest/debates.py --limit 500          # -> data/normalized/debates.jsonl

# Full backfill. Offline, maps the committed archive under data/<year>/: ~14s, no network.
python3 ingest/debates.py --source sittings

# A date window (either form; ISO preferred)
python3 ingest/debates.py --source sittings --since 2026-01-01
```

Three outputs land in `data/normalized/`:

| file | what it is |
|---|---|
| `debates.jsonl` | the documents — one record per report |
| `members.jsonl` | the roster: one `member` record per MP, with the speaker strings this run saw resolving to them |
| `debates.report.json` | run report: counts, skipped/error categories, low-coverage sittings, unresolved speakers |

**Live vs. offline.** `--source live` (the default) fetches from `sprs.parl.gov.sg` and
caches every raw response under `data/raw/`, so a rerun is cheap — add `--force` to
re-fetch. `--source sittings` reads only what is already committed, needs no network, and
covers the same 331 sittings; use it for a deterministic full backfill. Both paths converge
on the same `map_report()`, so they cannot drift: `--limit` takes "the first N in output
order" (sitting date ascending), not a random sample.

**The run does not trust itself.** Every written row is validated against
`docs/schema.json` before it is counted, and the process exits non-zero on the first
invalid row rather than reporting a row count it cannot stand behind. The report also
surfaces the things that would otherwise be silent: sittings where the listing API dropped
rows (`coverage_ratio < 0.95`), records refused as the other partition of the corpus, and
speaker strings that matched no roster member. Read the report, not just the exit code.

The report's `turns_by_kind` is **this partition's** count (sections other than
`bill`/`motion`), which is not the corpus-wide table in `docs/rag_spec.md` §3.3 — the report
carries a `resolution_counts_scope` field saying so, and §3.3's table is reproduced over the
whole corpus by `docs/probes/ingest_resolution_ledger.py`.

**Speaker resolution.** Each turn keeps its verbatim `speaker` string *and* a resolved
`member_id`, so a wrong attribution stays visible in the record rather than being
undetectable. Unresolvable strings are kept as served and counted in the report. Chamber
officers (Speaker, Chairman) are *not* members and never get an id. Two things worth knowing
before you read a report and think something is broken:

- The source glues an invisible mark (a BOM) to ~10 speaker strings. Resolution strips
  invisible marks **only for matching**; the stored `speaker` keeps exactly what the source
  served. Without this, `'\ufeffMdm Speaker'` was not recognised as an officer and the turn
  was miscounted as an unresolved person.
- The name-variant fallback resolves `Mr Mohd Fahmi Bin Aliman` to the roster's
  `Mohd Fahmi Aliman`, and refuses when a match would be ambiguous. Genuine alias cases that
  it correctly refuses (rather than guessing) include `Mr Alex Yam Ziming` (17 turns) and
  `Mr Edwin Tong` (1 turn) against the roster's `Edwin Tong Chun Fai`.

Tests and drift checks:

```bash
python3 docs/probes/ingest_gates.py                    # ALL of the below, one verdict
python3 -m pytest ingest/test_debates.py -q            # 74 mapping tests, offline fixture
python3 docs/probes/ingest_verify.py                   # re-derives every claim from the written files
python3 docs/probes/ingest_fixture_check.py            # the fixture still matches the corpus
python3 docs/probes/ingest_partition_check.py          # debates + legislation cover the corpus exactly
python3 docs/probes/ingest_since_check.py <jsonl> <since>   # a --since run honoured its filter
python3 docs/probes/ingest_resolution_ledger.py        # spec §3.3's table, corpus-wide
python3 docs/probes/ingest_fixture_build.py --write    # regenerate the fixture (only if it drifts)
```

`ingest_verify.py` and `ingest_gates.py` are the ones to run before trusting a number here:
the first reads the written JSONL off disk and re-derives every claim above without sharing
code with the run path, and the second runs all of the checks and prints one verdict.

**Full backfill, start to finish.** The archive is already complete (331 sittings,
20,884 reports, 2016-01-15 → 2026-08-05), so a backfill normally means "regenerate the
normalized corpus", not "re-fetch the decade":

```bash
python3 ingest/debates.py --source sittings        # ~14s, no network; 19,089 documents
python3 ingest/legislation.py                      # bills + motions -> legislation.jsonl
python3 docs/probes/ingest_verify.py               # every row validates, read off disk
```

To pick up sittings the archive does not yet have, run `--source live`. It enumerates the
sitting files already present to learn which days to ask about, so add the new sitting files
first (or run the project's fetcher). It caches each sitting's raw responses under
`data/raw/sittings/`, so an interrupted backfill resumes where it stopped and a rerun costs
no network; `--force` re-fetches. The run prints per-sitting progress and reports every
failed report fetch by id rather than quietly emitting a row with empty text — empty text
already means something specific here ("the source had none").

**Live runs are snapshots, and the report says how far they drift.** Measured on the 21
cached sittings: the live enumeration returned 16 records the committed archive does not have
(`attendance-*`, `ptba-*`, and 11 `written-answer-*` on one day), while the archive held 136
the live cache lacked — 14 of them absent from the day's transcript entirely
(`docs/probes/ingest_attendance_gap.py`). So `--source live` and `--source sittings` do not
produce identical corpora, and a live run writes `live_vs_archive` into its report saying so.
`--source sittings` is the reproducible path and is what every corpus count in
`docs/rag_spec.md` rests on; use it for anything that must be reproducible.
...
├── ingest/
│   ├── debates.py                 archive -> normalized JSONL (the RAG corpus)
│   ├── rag_schema.py              schema.json, enums and converters, shared
│   ├── fixtures.py                corpus-derived fixture for the mapping tests
│   └── test_debates.py            record-mapping tests (offline)
├── data/
│   ├── manifest.json              what exists, what is summarised, what is stale
│   ├── sittings.json              flat site-facing index
│   ├── normalized/                ingest output (debates.jsonl, members.jsonl, report)
│   ├── raw/                       raw API cache, so reruns are cheap
